...
export function RequirementDetailPage() {
  ...
  const loadDetail = useCallback(async () => {
    if (!id) return;
    try {
      const res = await fetchRequirementDetail(id, isAdmin);
      setData(res);
    } catch {
    ...
    <div className="flex flex-col gap-8 animate-rise pb-10">
      ...
      <div className="space-y-10">
        {data.results.map((entry) => (
          <div key={entry.matchingId} className="space-y-6">
            {entry.items.map((item, idx) => (
              <div
                key={idx}
                className="grid lg:grid-cols-12 gap-6 bg-card rounded-2xl p-6 border shadow-sm"
              >
                {/* LEFT: REQUIRED ITEM */}
                <div className="lg:col-span-4 flex flex-col justify-center">
                  <div className="flex items-center gap-2 text-[10px] uppercase font-black text-muted-foreground tracking-widest mb-3">
                    <Search className="h-3 w-3" /> Target Item
                  </div>
                ...
                <div className="lg:col-span-8 bg-muted/30 rounded-xl p-4 ring-1 ring-border/50">
                  ...
                  <div className="space-y-3 max-h-75 overflow-y-auto pr-2 custom-scrollbar">
                    {item.matches.length === 0 ? (
                      <div className="h-24 flex items-center justify-center text-xs text-muted-foreground italic border border-dashed rounded-lg">
                      ...
                      item.matches.map((match, mIdx) => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div
                              className={`flex flex-col items-center justify-center h-12 w-14 rounded-lg text-xs font-black ${mIdx === 0 ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground"}`}
                            >
